...
public class ImageMergeUtil {
     ...
     * （注意：必须两张图片长宽一致）
     */
    public static File mergeImageToFile(BufferedImage[] images, int type , String absolutePath) throws IOException {
        ...
        if (type == 1 && newWidth < 1) {
            throw new RuntimeException("图片计算宽高失败");
        }
        ...
        try {
            ...
            File result = new File(absolutePath.substring(0, absolutePath.lastIndexOf(".")) + ".png");
            //输出想要的图片
            ImageIO.write(ImageNew, "png", result);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("图片拼接失败");
        }
    }
